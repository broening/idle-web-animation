...
| 2 | Anticipation | **token** | The timing is real — `pulse(t − start + 0.14, 0.14)` fires in exactly the 140 ms before a blink. The effect is not. There is no lid layer, so it scales the whole eye layer by 1% about a pivot 229 px below the eyes. Measured: the eye band **moves up 8.5 px** and **changes height 2.1 px**. The described effect is the small one. The second blink of a double blink gets no anticipation at all. A page that drives `blink` from a camera switches the widening off while it does, since there is nothing to anticipate. |
| 3 | Staging | **judgement** | Which part carries the eye, what the pose says. No code decides this. |
| 4 | Straight ahead vs. pose to pose | **not implemented** | The engine is procedural: every layer is a function of `t`. There is not one pose or keyframe in the schema. That is neither of the two techniques, and calling determinism "pose to pose" was a rename, not an implementation. Moods (`states`) do not change the verdict. A mood is an input like the pointer, not a pose placed on a timeline: the page asks for `sad`, both moods are solved as functions of `t`, and a blend mixes them. The figure is a function of time plus its inputs, and keyframes and timelines stay out. |
...
| 8 | Secondary action | **warned, not enforced** | The differing default periods are defaults, nothing more, and the engine never perturbs a period. Two parts on one period keep the same distance between their peaks forever — one mechanism, exactly what the principle warns against. The Kriegerin shows it: the bear's head and the warrior's hair are two bodies and both sway at 7.5 s. The studio's Motion card now says so for the selected layer: the same period, or exactly double or half, anywhere in the figure. `gaze` against `gaze` is left out, because eyes are meant to look together. Whether a pair should move as one stays the author's call. |
| 9 | Timing | **partial** | Only `breathe`, `sway`, `gaze` and `glow` have a `period`. `blink` has `interval`, `flipbook` has `every` and `fps`, `charge` has `cycle`, `drift` has `every` and `life`. The studio draws a slider for every number a motion reads, written or not, so a motion without `period` shows its default instead of hiding it. |
...
| 11 | Solid drawing | **enforced** | Layers displace by their own `depth` as the pointer moves, so a flat stack reads as a space. `trackPointer` listens to pointer events, so a finger dragging over the figure steers it as a mouse does; lifting it recentres. |
